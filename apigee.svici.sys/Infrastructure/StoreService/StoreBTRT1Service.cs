...
{
    public class StoreBTRT1Service
    {
        private static readonly Serilog.ILogger _logger = Log.ForContext<StoreBTRT1Service>();

        public string GetBTRT1(ResponseServiceConfig setting, BTRT1_Dto data)
        {
            ...
            try
            {
                ...
                //string path = Path.Combine("Utilities/Data/RequestTemplates/StoreTemplates/BTRT1_Credit_Req_Resp.xml");
                ...
                if (xmlTemplate != null)
                {
                    ...
                    xml = string.Format(xmlTemplate,
                        ...
                        //document block
                        data.customerBlock.DocumentType,
                        data.customerBlock.Number,
                        //customer block
            ...
            {
                _logger.Error($"StoreBTRT1Service.GetBTRT1 =======> " +
                    $"Expection msg: {ex.Message}");
                return xml;
            }
        }
        private string getAdditionalService(ResponseServiceConfig setting, List<AdditionalServiceBlock> _list)
        {
            try
            {
            ...
            {
                _logger.Error($"StoreBTRT1Service.getAdditionalService =======> " +
                   $"Expection msg: {ex.Message}");
                return null;
            }
        }
